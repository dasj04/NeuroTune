...
if not os.path.exists("data/eeg_session.npy") or not os.path.exists("data/sampling_rate.npy"):
    import pickle
...
if not all(os.path.exists(p) for p in [
    ...
    _eeg_d = np.load("data/processed/eeg_data.npy")
    _lbls  = np.load("data/processed/labels.npy")
    _fs_p  = int(np.load("data/sampling_rate.npy")[0])

    def _bpow_fn(sig, lo, hi):
        nyq = _fs_p / 2.0
        b, a = butter(4, [lo/nyq, hi/nyq], btype="band")
        return np.mean(filtfilt(b, a, sig) ** 2)

    _feats, _flabels, _t0, _a0, _b0 = [], [], [], [], []
    _ws = 2 * _fs_p
    for _ti, _tr in enumerate(_eeg_d):
        _sig, _i = _tr[0], 0
        while _i + _ws <= len(_sig):
            _a = _bpow_fn(_sig[_i:_i+_ws], 8, 13)
            _b = _bpow_fn(_sig[_i:_i+_ws], 13, 30)
            _feats.append([_a, _b, _a / (_b + 1e-10)])
            _flabels.append(_lbls[_ti])
            if _ti == 0:
                _t0.append((_i + _ws / 2) / _fs_p)
                _a0.append(_a)
                _b0.append(_b)
            _i += _fs_p
    os.makedirs("data/processed", exist_ok=True)
...
if not os.path.exists("data/classified_states.npy"):
    ...
    _Xtr, _, _ytr, _ = train_test_split(_X, _y, test_size=0.2, random_state=42)
    _sc  = StandardScaler()
    _clf = SVC(kernel="rbf")
    _clf.fit(_sc.fit_transform(_Xtr), _ytr)
    _ap  = np.load("data/alpha_powers.npy")
    _bp  = np.load("data/beta_powers.npy")
    _r   = _ap / (_bp + 1e-10)
    _raw = _clf.predict(_sc.transform(np.column_stack([_ap, _bp, _r])))
    _k = 2
    _sm = np.array([
        1 if np.mean(_raw[max(0,i-_k):min(len(_raw),i+_k+1)]) >= 0.5 else 0
        for i in range(len(_raw))
    ])
    np.save("data/classified_states.npy", _sm)
...
ax1.set_title("① Raw EEG Signal (Simulated — 60 Seconds)", fontweight="bold")
